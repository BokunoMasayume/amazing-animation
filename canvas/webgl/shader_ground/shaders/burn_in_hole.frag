...
float circle(vec2 st , vec2 center){
	return distance(fract(st) , center)*(1. / u_time * 20.);
}
...
void main(){
	...
	vec4 grandColor = mix( vec4( 0. , 0. , 0. ,1.) ,vec4(1.,1.,1.,0.) ,smoothstep(.58,1.,color));
	if(grandColor.a > .994 && grandColor.a <1.){
		//grandColor = vec4( 0.985,0.140,0.000 ,1.); //orange
		grandColor = mix( vec4( 0.985,0.140,0.000 ,1.) , vec4(0.985,0.696,0.285 , 1.) , smoothstep(.994, .999 , grandColor.a) );
		//E4550A
	}
		...
		//grandColor = vec4(grandColor.xyz * grandColor.a + bg.xyz * (1. - grandColor.a) ,1.);
	}
